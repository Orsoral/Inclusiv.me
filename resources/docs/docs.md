...
The `home-routes` handler in the `inclusiv.me.routes.home` namespace
...
The `home-page` function will in turn call the `inclusiv.me.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `inclusiv.me.handler` namespace:
...
Request middleware functions are located under the `inclusiv.me.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `inclusiv.me.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `inclusiv.me.db.core` in some other namespace.
